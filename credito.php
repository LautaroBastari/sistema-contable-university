...
<body>
    ...
    $consulta_nota_credito = $conexion->query("SELECT producto, cantidad FROM nota_credito WHERE id_venta=$id");
    if ($consulta_nota_credito){
        if ($consulta_nota_credito->num_rows > 0) {
            $primer_dato = $consulta_nota_credito->fetch_assoc();
            ...
            $mensaje = urlencode("No se encontraron registros en la nota de crédito.");
            header("Location: ventas.php?creditos_vacios=1");
            exit();
        }
    ...
    <div class="container mt-2" style="width: 700px;">
        <div class="d-flex ">
            ...
            <div class="container text-center border ">
                <h2 class="mt-3"><b>NOTA DE CRÉDITO</b></h2>
                <p>Número Nota: <?= $id ?></p>
        ...
        $importe_neto = 0;
        $iva = 0;
        while ($datos_nota_credito = $consulta_nota_credito->fetch_assoc()){
            $producto = $datos_nota_credito["producto"];
            $cantidad = $datos_nota_credito["cantidad"];
            ...
            if ($datos_producto = $consulta_producto->fetch_assoc()){
                ...
                $precio_final_producto = $precio_total + $precio_total * 0.21; // Nota de crédito: descuento IVA
        ...
        <div class="d-flex flex-column align-items-end border border-danger">
            ...
            <?php } ?>
            <div class="me-4"><p><strong>Total: $<?= number_format($total, 2) ?></strong></p></div> <!-- Total con IVA descontado -->
        </div>
